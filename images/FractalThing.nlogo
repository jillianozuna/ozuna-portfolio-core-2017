...
to setup
  ...
  [
    ...
    setxy 0 0
    set color 15
    set modulus ((Cheese / 10 ) * max-pycor)
    pen-down
  ]
  reset-ticks
end
...
to grow
  hatch 1
    [ fd modulus
      set modulus ( (Cheese / 10) * modulus) ]  ; new turtle's modulus is half its parent's
end
...
to go
  ...
  [
    ...
    [
      grow
      right 360 / Edges  ; turn counter-clockwise to draw more legs
      set color (color + 10)
    ]
